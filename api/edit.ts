...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { imageData, prompt, hotspot } = req.body;
...
User Request: "${prompt}"
${hotspot ? `Edit Location: Focus on the area around pixel coordinates (x: ${hotspot.x}, y: ${hotspot.y}).` : ''}
...
Output: Return ONLY the final edited image. Do not return text.`;
    ...
    return res.status(500).json({ error: 'Failed to process image edit' });
  }
}